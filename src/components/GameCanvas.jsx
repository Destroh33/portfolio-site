...
const RAIL_MIN_VEL  = 200
const PW = 20, PH = 52
const FLIP_SPIN = Math.PI * 3
const WIPE_DUR  = 1.5
const GOOD_LAND = 1   // more forgiving landing angle
...
function drawGround(ctx, groundY, lW, camX) {
  ...
  const off2 = Math.round(-((camX + TW / 2) % TW))
  ...
  ctx.fillRect(0, groundY, lW, PX)
  ctx.fillRect(0, groundY + PX + TH, lW, PX)
  ctx.fillRect(0, groundY + PX + TH + PX + TH, lW, PX)
...
function drawArtThumb(ctx, cx, bottomY, artImg, fw = FRAME_W, fh = FRAME_H) {
  const sc = fw / FRAME_W
  const PX = Math.max(1, Math.round(3 * sc))
  const B  = Math.max(4, Math.round(12 * sc))
...
function drawPlatform(ctx, plat, camX, cW, groundY) {
  ...
  if (plat.type === 'rail') {
    ...
    ctx.fillRect(px1, plat.y + plat.h, 6, postH)
    if (plat.w > 60) ctx.fillRect(px2, plat.y + plat.h, 6, postH)
    ctx.strokeStyle = '#2a1a0a'; ctx.lineWidth = 1
    ctx.strokeRect(px1 + .5, plat.y + plat.h + .5, 5, postH - 1)
    if (plat.w > 60) ctx.strokeRect(px2 + .5, plat.y + plat.h + .5, 5, postH - 1)
...
function drawSkater(ctx, player, animFrame, spriteSheet, camX) {
  ...
  ctx.save()
  ctx.imageSmoothingEnabled = false
  ctx.translate(sx, by - DST_H / 2)
  ctx.rotate(player.angle || 0)
  if (player.facing === 'left') ctx.scale(-1, 1)
  if (spriteSheet.complete && spriteSheet.naturalWidth > 0) {
    ctx.drawImage(
      spriteSheet,
      frame * SRC_W, 0, SRC_W, SRC_H,
      -DST_W / 2, -DST_H / 2, DST_W, DST_H
    )
  }
  ctx.restore()
}
...
function buildSkatepark(lW, groundY) {
  const railW = Math.round(lW * 0.16)
  const railH = 10
  const railX = Math.round(lW * 0.50) - Math.round(railW / 2)
  const railY = groundY - 20

  return {
    worldW: lW,
    platforms: [
      { x: 0, y: groundY, w: lW, h: 80, type: 'ground' },
      { type: 'rail', x: railX, y: railY, w: railW, h: railH },
    ],
    groundY,
  }
}
...
function updatePhysics(state, dt, lW, groundY, platforms, skateparkWorldW) {
  ...
  const isSkate = skateparkWorldW > 0
  ...
  const spaceJust = spaceHeld && !state.spacePrev
  state.spacePrev = spaceHeld

  // While on a rail, lock direction to prevent reversals
  let effectiveDir = dir
  if (isSkate && player.onRail) {
    const currentDir = Math.sign(player.vx) || (player.facing === 'right' ? 1 : -1)
    // Only allow input that continues in the same direction
    effectiveDir = dir === -currentDir ? 0 : dir
  }

  if (effectiveDir !== 0) {
    const accel = player.onGround ? ACCEL_GND : ACCEL_AIR
    player.vx += effectiveDir * accel * dt
    player.vx = Math.sign(player.vx) * Math.min(Math.abs(player.vx), MAX_SPEED)
    if (!isSkate || player.onGround) player.facing = effectiveDir > 0 ? 'right' : 'left'
    player.walkTarget = null
  } else if (!isSkate && player.walkTarget !== null) {
    const dx = player.walkTarget - (player.x + PW / 2)
    if (Math.abs(dx) < 5) {
      player.vx = 0
      player.walkTarget = null
    } else {
      player.vx = (dx > 0 ? 1 : -1) * MAX_SPEED * 0.65
      player.facing = dx > 0 ? 'right' : 'left'
    }
  } else {
    const slippery = player.onRail || (isSkate && player.onGround && player.currentSlope)
    const friction = (slippery ? FRICTION_RAIL : FRICTION_GND) * dt
    if (Math.abs(player.vx) <= friction) player.vx = 0
    else player.vx -= Math.sign(player.vx) * friction
  }

  // Rail physics: maintain minimum velocity
  if (isSkate && player.onRail) {
    const railDir = Math.sign(player.vx) || (player.facing === 'right' ? 1 : -1)
    if (Math.abs(player.vx) < RAIL_MIN_VEL) {
      player.vx = railDir * RAIL_MIN_VEL
    }
    player.facing = railDir > 0 ? 'right' : 'left'
  }

  if (isSkate && player.onGround && player.currentSlope && player.currentSlope.angle > 0) {
    const gravAlong = GRAVITY * Math.sin(player.currentSlope.angle) * dt
    if (player.currentSlope.side === 'left')  player.vx += gravAlong
    if (player.currentSlope.side === 'right') player.vx -= gravAlong
    player.vy = 0
  }

  if (isSkate && !player.onGround && spaceJust && player.angularVel === 0) {
    player.angularVel = player.facing === 'right' ? -FLIP_SPIN : FLIP_SPIN
  }

  if (isSkate && !player.onGround) {
  ...
  const slopeHandlesVert = isSkate && player.onGround && player.currentSlope
  if (!slopeHandlesVert) {
    player.vy = Math.min(player.vy + GRAVITY * dt, 980)
  }

  player.x += player.vx * dt
  player.x = Math.max(0, Math.min(lW - PW, player.x))

  const prevY = player.y
  player.y += player.vy * dt

  const prevOnGround = player.onGround
  const prevOnRail   = player.onRail
  const prevSlope    = player.currentSlope
  ...
  if (platforms.length > 0) {
    const footProbeX = player.x + PW * 0.5
    const footY = player.y + PH
    const prevFootY = prevY + PH

    let matchedSlope = null

    if (isSkate) {
      let bestLanding = null
      let bestRolling = null
      let currentSlopeCandidate = null
      ...
      for (const seg of platforms) {
        ...
        if (footProbeX < xMin - 1 || footProbeX > xMax + 1) continue

        const dx = seg.x2 - seg.x1
        const rawT = dx === 0 ? 0 : (footProbeX - seg.x1) / dx
        const t = clamp(rawT, 0, 1)
        const surfY = seg.y1 + t * (seg.y2 - seg.y1)

        const feetTopY = player.y + PH * 0.75
        ...
        const rollingOntoSlope =
          prevOnGround &&
          footY >= surfY - 10 &&
          feetTopY <= surfY + PH * 0.25

        const stayingOnCurrentSlope =
          prevSlope === seg &&
          Math.abs(footY - surfY) <= 12

        if (stayingOnCurrentSlope) {
          currentSlopeCandidate = { seg, surfY }
        }

        if (crossingDown) {
          const landingScore = Math.abs(footY - surfY)
          if (!bestLanding || landingScore < bestLanding.score) {
            bestLanding = { seg, surfY, score: landingScore }
          }
        }

        if (rollingOntoSlope) {
          const comingFromRamp = prevSlope && prevSlope.side !== 'flat'
          const isFlatDeck = seg.side === 'flat'

          if (!(comingFromRamp && isFlatDeck)) {
            if (!bestRolling || surfY < bestRolling.surfY) {
              bestRolling = { seg, surfY }
            }
          }
        }
      }

      matchedSlope = currentSlopeCandidate || bestLanding || bestRolling
    }

    if (matchedSlope) {
      const seg = matchedSlope.seg
      const surfY = matchedSlope.surfY

      player.y = surfY - PH
      player.vy = 0
      player.onGround = true
      player.onRail = false
      player.currentSlope = seg

      player.angle =
        seg.side === 'left' ? seg.angle :
        seg.side === 'right' ? -seg.angle :
        0

      player.angularVel = 0
    } else {
      for (const plat of platforms) {
        if (plat.type === 'slope') continue
        ...
        if (plat.type === 'ground') {
          ...
          if (
            player.vy >= 0 &&
            pBottom >= platTop &&
            pBottom <= platTop + plat.h &&
            prevY + PH <= platTop + 16
          ) {
            player.y = platTop - PH
            player.vy = 0
            player.onGround = true
            player.onRail = (plat.type === 'rail')
            break
          }
        }
      }
  ...
  // Launch off the curved lip without catapulting straight up.
  if (isSkate && !player.onGround && prevOnGround && prevSlope && prevSlope.side !== 'flat') {
    const dx = prevSlope.x2 - prevSlope.x1
    const dy = prevSlope.y2 - prevSlope.y1
    const len = Math.hypot(dx, dy) || 1

    const ty = dy / len
    const speed = Math.abs(player.vx)

    let launchVy = -speed * Math.abs(ty)
    launchVy = Math.max(launchVy, -220)

    if (player.vy > launchVy) {
      player.vy = launchVy
    }
  ...
  if (isSkate && player.onGround && !player.currentSlope) {
    player.angle = 0
  }

  if (isSkate && player.onGround && !prevOnGround && !prevOnRail && !prevSlope) {
    const norm = ((player.angle % (2 * Math.PI)) + 2 * Math.PI) % (2 * Math.PI)
    const dist = Math.min(norm, 2 * Math.PI - norm)

    state.badLanding = dist > GOOD_LAND
    player.angle = 0
    player.angularVel = 0
  }
...
export default function GameCanvas({ room, onRoomChange, onOpenModal, paused }) {
  ...
  useEffect(() => {
    ...
    artImgs['portrait'] = new Image()
    artImgs['portrait'].src = `${B}images/gcprofile.png`
    ...
    let entities = [], platforms = [], skateparkWorldW = 0, skateparkData = null
      ...
      if (currentRoom === 'home') {
        for (const lr of homeLinkRects) {
          if (clickX >= lr.x && clickX <= lr.x + lr.w && clickY >= lr.y && clickY <= lr.y + lr.h) {
            window.open(lr.url, '_blank', 'noopener,noreferrer')
            return
          }
        }
        state.player.walkTarget = clickX
        return
      }
    ...
    function loop(now) {
      ...
      if (!pausedRef.current) {
        if (pendingSpawnRef.current !== null) {
          ...
          const isSkateSpawn = roomRef.current === 'skatepark'
          ...
          state.player.angularVel = 0
          state.player.wiping = false
          state.player.wipeTimer = 0
          state.player.currentSlope = null
          state.transitionCD = 0.9
          state.camera.x = 0
          state.prevOnGround = true
          state.prevOnRail = false
          if (isSkateSpawn) {
            state.trickMsg = 'PRESS JUMP IN MIDAIR TO FLIP'
            state.trickMsgTimer = 3.0
          }
        ...
        if (roomRef.current === 'skatepark') {
          ...
          if (!p.onGround) {
            state.airTime  += dt
            state.airAngle += Math.abs(p.angularVel) * dt
          } else if (p.onGround && !wasOnGround) {
            if (state.airAngle > 0) {
              const flips = Math.floor(state.airAngle / (2 * Math.PI))
              if (flips > 0) {
                const pts = flips * 500 * state.combo
                state.score += pts
                if (!p.onRail) {
                  state.combo = 1
                } else {
                ...
                state.trickMsg = flips + (flips === 1 ? ' FLIP' : ' FLIPS') + '!  +' + pts
                state.trickMsgTimer = 2.5
              } else if (!p.onRail && !state.badLanding) {
                state.combo = 1
              }
            } else if (!p.onRail && !state.badLanding) {
              state.combo = 1
      ...
      if (roomRef.current === 'skatepark') {
        for (const plat of platforms) {
          if (plat.type === 'rail') drawPlatform(ctx, plat, camX, logicalW, groundY)
        }
      ...
      if (entities.length > 0) {
        ...
        const effIS = Math.round(ICON_SIZE * itemScale)
        const effFW = Math.round(FRAME_W   * itemScale)
        const effFH = Math.round(FRAME_H   * itemScale)
        ...
        entities.forEach((e, i) => {
          ...
          if (e.type === 'project') {
            ...
            ctx.fillStyle = '#2a1a0a'
            ctx.font = `${lblSz}px "Press Start 2P", monospace`
            ctx.textAlign = 'center'
